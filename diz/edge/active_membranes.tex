...
In this subsection we will focus on the problem of deciding whether there is a halting computation. Recall that halting computation has no applicable rule in the last configuration.
First, we will reduce the reachability problem to this problem. The reachability problem is to determine, for a given configuration $C$, whether there exists a computation from $C_0$ to $C$. Then, reachability of register machines, which is undecidable, can be reduced to the reachabililty of active P systems.

For a given P system $\Pi$ and a target configuration $C$ we will construct a P system $\Pi^\prime$ such that there is a halting computation of $\Pi^\prime\Leftrightarrow\linebreak C$ is reachable for $\Pi$. Suppose $\Pi = (\Sigma, C_0, R_1, \ldots, R_m)$, where $C_0 = (T_0, l_0, c_0)$ and $C = (T, l, c)$. Then we will construct $\Pi^\prime = (\Sigma^\prime, C_0^\prime, R_1^\prime, \ldots, R_m^\prime)$, where:

\begin{itemize}
  \item $\Sigma^\prime = \Sigma\cup\{\xi_d|d\in V(T)\}$,
  \item $C_0^\prime = (T_0, l_0, c^\prime)$, where $\forall d\in V(T_0)\setminus r_{T_0}: c^\prime(d) = c_0(d)$ and $c^\prime(r_{T_0}) = c_0(r_{T_0})\cup\{\xi_{r_T}\}$,
  \item $\forall i\in\{1,\ldots, m\}: R_i^\prime = R_i\cup\{\xi_d c(d)\rightarrow(\xi_{d^\prime}\downarrow_{l(d^\prime)}|d^\prime\in V(T),parent_T(d^\prime)=d)|d\in V(T), l(d)=i\}$.
\end{itemize}

The $\xi_d$ objects are called verifiers, they are intended to verify if the contents of the membrane corresponds to the contents in the target configuration $C$. If the current configuration is not $C$, it is not guaranteed that the verification can complete. After the verification of a membrane contents it descends down into all child membranes using single rule in order to verify other parts of the membrane structure.
...
In this subsection we will focus on the opposite problem: whether there is a computation that is infinite. Due to technical difficulties we will provide a proof only for active P systems with limit on the total number of membranes.

...
\newpage
\note{
  Keď stromy nie sú izomorfné, tak podľa preorderu dostanú iný order number, a teda v kódovaní bude nenulová iná časť. Tým pádom nebude platiť ani $enc(C_1) < enc(C_2)$ ani $enc(C_2) < enc(C_1)$. Inými slovami - ak sú kódovania porovnateľné $enc(C_1) < enc(C_2)$ alebo $enc(C_2) < enc(C_1)$, potom už nutne musí byť nenulová tá istá časť a teda stromy sú izomorfné.

...
\begin{abstract}
An universal variant of sequential P systems with active membranes is investigated and we show that existence of infinite computation is decidable. The standard construction of coverability tree is extended to provide an algorithm for detecting infinite loop.
...
There are number of results for sequential P systems, without priorities they are equivalent to VAS \cite{Ibarra05Active}, hence not universal. However priorities, inhibitors and other modifications can increase the computation power. It seems there is a link between universality and ability to zero-check \cite{Alhazov12Properties}. But if we allow membranes to be created unlimited number of times, the universality can be achieved without the zero-check extension \cite{Ibarra05Active}. Such P systems are called active P systems.

In section \ref{sec:preliminaries} we will recall some basic notions from formal languages, multisets and graph theory. Then in section \ref{sec:p systems} we will introduce membrane structure, formally define membrane configuration and active P system, because standard definitions were not convenient for the formal proofs. Section \ref{sec:termination_problems} contains main theorems with proofs.

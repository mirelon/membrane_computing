...
\begin{abstract}
An universal variant of sequential P systems with active membranes is investigated and we show that existence of infinite computation is decidable. The standard construction of coverability tree is extended to provide an algorithm for detecting infinite loops.
On the other hand, the existence of a halting computation is shown to be undecidable, as it can be reduced to reachability of register machines.
\end{abstract}
...
In sequential P systems, only one rewriting rule is used in each step of computation. Without priorities, they are equivalent to vector addition systems \cite{Ibarra05Active}, hence not universal. However priorities, inhibitors and other modifications can increase the computation power. It seems that there is a link between universality and ability to zero-check \cite{Alhazov12Properties}. But there are also variants, where universality can be achieved without checking for zero, i.e. if we allow membranes to be created unlimited number of times \cite{Ibarra05Active}. Such P systems are called active P systems. Contrary, if we place a limit on number of times a membrane is created, such active P system is still equivalent to vector addition systems.

In section \ref{sec:preliminaries} we will recall some basic notions from formal languages, multisets and graph theory. Then in section \ref{sec:p systems} we will introduce membrane structure, formally define membrane configuration and active P system, because standard definitions are not convenient for our formal proofs. Section \ref{sec:termination_problems} contains main theorems with proofs.
...
We will reduce the problem of deciding existence of halting computation to the problem of reachability of a given configuration. For a given P system $\Pi$ and a target configuration $C$ we will construct a P system $\Pi^\prime$ such that there is a halting computation of $\Pi^\prime$ iff the $\Pi$ can reach configuration $C$.
Suppose $\Pi = (\Sigma, C_0, R_1, \dots R_m)$ and $C = (T, l, c)$. Then $\Pi^\prime = (\Sigma^\prime, C_0^\prime, R_1^\prime, \dots R_m^\prime)$, where:
\begin{itemize}
...
At the start, every membrane contains an additional $\omega$ and a rule $\omega\rightarrow\omega$, so the computation will halt only if these $\omega$s are erased.
The $\xi_{r_T}$ object is called verifier, it verifies if the contents of each membrane corresponds to the contents in the target configuration $C$. After this verification it descends down into child membranes for the verification of other parts of the membrane structure. It also replaces $\omega$ with $\sigma$, which stands as a trace symbol, meaning that the contents of this membrane has already been verified. $\sigma$ objects are used to eliminate dissolution of membranes when $\xi$ object has already verified the contents. Such dissolution would cause to two $\sigma$s to meet in the same membrane which would cause an infinite computation.
...
\begin{proof}
  After the application of rule $\xi_d\omega c(d)\rightarrow\sigma\xi_{d^\prime}\downarrow_{l(d^\prime)}$, the object $\sigma$ remains in the membrane. There is no interaction of $\sigma$ with other objects, only with another $\sigma$. If a child membrane $d^\prime$ is dissolved with a rule $u\rightarrow w\delta$, then two symbols $\sigma$ will meet in the membrane $d$ and the computation will not halt, which contradicts the fact that the computation is halting. \qed
\end{proof}
...
\begin{proof}
  After the application of rule $r = \xi_d\omega c(d)\rightarrow\sigma\xi_{d^\prime}\downarrow_{l(d^\prime)}$ in membrane $d$, there won't be any of $\xi$ symbols present in the membrane, because they are only sent into child membranes, which cannot be dissolved because of lemma \ref{no_dissolving_after_check_lemma}. So there will be no more of rule $r$ applied in membrane $d$. The newly created membrane will never receive any of $\xi$ objects, so the object $\omega$ will never be erased and the computation will not halt, which contradicts the fact that the computation is halting. \qed
\end{proof}

\begin{lemma}
\label{check_at_last_lemma}
  For all halting computations of $\Pi^\prime$ there is a halting computation, where for every membrane $d$ the last rule used is $r = \xi_d\omega c(d)\rightarrow\sigma\xi_{d^\prime}\downarrow_{l(d^\prime)}$.
\end{lemma}

\begin{proof}
  Consider membrane $d$: let $C_1$ be the configuration before the application of $r$, $C_2$ the configuration after the application of $r$ and $C_3$ the halting configuration. The sequence of steps from $C_2$ to $C_3$ cannot use $\sigma$, so the same sequence can be applied starting from $C_1$ instead of $C_2$. It would result in a configuration $C_4$, where contents of $d$ is exactly $\xi_d\omega c(d)$. So the rule $r = \xi_d\omega c(d)\rightarrow\sigma\xi_{d^\prime}\downarrow_{l(d^\prime)}$ can be applied, resulting in configuration, where $d$ contains exactly one $\sigma$ and nothing else, so no more rule will be applied in $d$. \qed
\end{proof}
...
\begin{proof}
  According to lemma \ref{check_at_last_lemma} there is also a halting computation $C_4$ where in every membrane $d$ the last used rule is $r = \xi_d\omega c(d)\rightarrow\sigma\xi_{d^\prime}\downarrow_{l(d^\prime)}$. So the corresponding computation in $\Pi$ will result in the configuration $C_5$, where every membrane $d$ contains exactly $c(d)$. If $C_5$ contains a membrane not present in $C$, it will contain the object $\omega$, which won't be reached by any of objects $\xi$ so the computation will not halt. If $C$ contains a membrane $d^\prime$ not present in $C_5$, then in $d = parent(d^\prime)$ at $C_5$ still contains $\omega$ because the rule $r = \xi_d\omega c(d)\rightarrow\sigma\xi_{d^\prime}\downarrow_{l(d^\prime)}$ could not be applied due to lack of a child membrane $d^\prime$. Hence $C = C_5$ and there is a computation of $\Pi$ that will result in $C$. \qed
\end{proof}

...
\providecommand{\event}{ICMNC 2017} % Name of the event you are submitting to
...
\usepackage{hyperref}
\usepackage{makeidx}
\let\labelindent\relax % http://tex.stackexchange.com/questions/170772/command-labelindent-already-defined
\usepackage{enumitem}

% Standard definitions used in llncs.cls and missing in journal.cls
...
Set membrane systems \cite{Kleijn11SetMembrane} are based on sets (of objects or rules) together with the associated set operations, rather than on multisets with multiset operations. An interesting property of maximal parallel steps in set membrane systems is that they are deterministic. The determinism relies on the absence of conflicts between applicable rules.
...
In this paper we work with sequential P systems with active membranes working on sets (Sequential active set P systems). The rules can modify the membrane structure by dissolving and creating new membranes. The skin membrane is not allowed to be dissolved, but object can be sent out through the skin membrane (e.g. to be observed by the surroundings). That is why we will define the configuration to include the membrane structure as well.

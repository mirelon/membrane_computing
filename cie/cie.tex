...
One group of variants consists of defining various constraints on rewriting rules. The original P systems use {\bf context rules} with any nonempty multiset on the left side and any multiset on the right side.  
{\bf Cooperative P systems} have only {\bf cooperative rules} that have at most two symbols on the left side.
{\bf Context-free P systems} have only {\bf context-free rules} that have only one symbol on the left side.
...
Another group of variants consists of defining various manners of using the rewriting rules. The original P systems that use maximal parallelism are called maximal parallel P systems.
{\bf Sequential P systems} are variants where in each step only a single rule in a single region is applied. The membrane and the rule are chosen nondeterministically. In \cite{Ibarra:2005:SPS:2111772.2111880} it is shown that the sequential P systems can be simulated by vector addition systems and are not universal. However, if we allow rules with membrane creation with unbounded number of membranes, they become universal.
